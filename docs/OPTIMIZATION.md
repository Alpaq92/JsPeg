...
| Arithmetic coding (SOF9) | ✅ supported | yes | `{ arithmetic: true }`; ~30–40% smaller, but ⚠️ **browsers can't display arithmetic JPEGs** (storage/interop only) |
...
is a SOF9 stream from a clean-room QM-coder encoder. Both are verified lossless
against our decoder **and** libjpeg. Trellis
...
| `arithmetic` | `false` | Transcode to an arithmetic-coded (SOF9) JPEG — smaller, but **not displayable in browsers**. Lossless; baseline input only. |
